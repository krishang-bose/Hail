...
export default function AuthButton({ usedCalls }: AuthButtonProps) {
	...
	const [usageToday, setUsageToday] = useState<number | null>(
		usedCalls ?? null,
	);
	...
	useEffect(() => {
		if (session?.user) {
			fetch("/api/usage")
				...
				.then((d) => {
					if (d?.used !== undefined) setUsageToday(d.used);
				})
				.catch(() => {});
		}
	}, [session]);
	...
	return (
		<div className="relative">
			...
			>
				...
				<span
					className="hidden sm:inline max-w-[100px] truncate font-medium"
					style={{ color: "var(--brown-3)" }}
				>
						...
						>
							<div className="flex items-center justify-between mb-1.5">
								...
								<span
									...
									style={{ color: isExhausted ? "#8B3A2A" : "var(--brown-2)" }}
								>
