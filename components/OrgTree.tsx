		...
		if (cat === "engineer" || cat === "recruiter") {
			...
			group.forEach((p) => {
				...
				if (cat === "founder" && byCategory["engineer"].length > 0) {
					const slice = byCategory["engineer"].splice(
						0,
						Math.ceil(
							byCategory["engineer"].length / Math.max(group.length, 1),
						),
					);
					slice.forEach((e) => {
						node.children!.push({
...
export default function OrgTree({
	...
	const draw = useCallback(() => {
		...
		const initialTransform = d3.zoomIdentity.translate(120, 50).scale(0.9);
		...
		const node = g
			...
			.attr("opacity", 0)
			.style("cursor", (d) =>
				d.data.category !== "company" ? "pointer" : "default",
			);
				...
				if (d.data.category !== "company") {
					d3.select(this)
						...
						.attr(
							...
							d.depth === 0
								? 30
								: d.data.category === "founder" || d.data.category === "cto"
									? 24
									: 20,
						)
		...
		node
			...
			.attr("r", (d) => {
				...
				if (d.data.category === "founder" || d.data.category === "cto")
					return 22;
				return 18;
			})
			.attr("fill", (d) => {
			...
			.attr("stroke-width", (d) =>
				d.data.id === selectedPersonId ? 2.5 : 1.5,
			);
		...
		node
			...
			.text((d) => {
				if (d.data.category === "company")
					return d.data.name.charAt(0).toUpperCase();
				return d.data.name
					.split(" ")
					.map((n) => n[0])
					.slice(0, 2)
		...
		node
			...
			.attr("dy", "-0.4em")
			.attr("fill", (d) =>
				d.data.id === selectedPersonId ? "#4A3B2C" : "#3D2E1E",
			)
			...
			.attr("font-family", "Inter, sans-serif")
			.text((d) =>
				d.data.name.length > 20 ? d.data.name.slice(0, 20) + "…" : d.data.name,
			);
		...
		node
			...
			.attr("font-family", "Inter, sans-serif")
			.text((d) =>
				d.data.role.length > 28 ? d.data.role.slice(0, 28) + "…" : d.data.role,
			);
