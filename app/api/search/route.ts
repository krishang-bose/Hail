...
function inferCategory(
	role?: string,
): "founder" | "cto" | "engineer" | "recruiter" {
	const r = (role ?? "").toLowerCase();
	if (r.includes("found") || r.includes("ceo") || r.includes("chief exec"))
		return "founder";
	if (r.includes("cto") || r.includes("chief tech") || r.includes("vp eng"))
		return "cto";
	if (
		r.includes("recruit") ||
		r.includes("talent") ||
	...
	try {
		...
		if (AUTH_ENABLED) {
			...
			if (session?.user) {
				...
				userId =
					(session.user as typeof session.user & { id?: string }).id ?? null;
			...
			if (cached.length === 1) {
				...
				if (secondaries.length > 0) {
					await Promise.all(
						secondaries.map(async (stub) => {
							...
							try {
								...
								/* ignore — stub insert failures are non-critical */
							}
						}),
					);
					...
					if (refreshed && refreshed.length > cached.length) {
						...
						if (AUTH_ENABLED && !isAdmin((await auth())?.user?.email)) {
							if (userId) await incrementUsage(userId);
							else if (ipHash) await incrementIpUsage(ipHash);
						}
		...
		const _allWebResults = clearbitAll; // alias used later in non-YC path
		const secondaryWebResults = clearbitAll.slice(1);

		if (ycCompany)
			console.log(`[YC] Found: ${ycCompany.name} (${ycCompany.batch})`);
		else
			console.log(
				`[YC] No match for "${q}" | Clearbit: ${clearbitAll.length} result(s)`,
			);
		...
		const storeSecondaryStubs = async () => {
			...
			console.log(
				`[Search] Storing ${secondaryWebResults.length} secondary stub(s)…`,
			);
			await Promise.all(
				secondaryWebResults.map(async (stub) => {
					if (!stub.name || !stub.domain) return;
					try {
						const { data: exists } = await supabaseAdmin
							.from("companies")
		...
		const fallback =
			clearbitAll.find(
				(r) => normalize(r.name ?? "").length / ql.length <= 1.5,
			) ?? null;

		const clearbitPrimary =
			clearbitAll.length === 0
				? null
				: (exactMatch ?? startsWithBest ?? containsMatch ?? fallback);

		if (clearbitPrimary) {
			console.log(
				`[Clearbit] Primary: "${clearbitPrimary.name}" | ${clearbitPrimary.domain}`,
			);
		} else {
			console.log(
				`[Clearbit] No close match for "${q}" — using query name directly`,
			);
		}
		const website =
			ycCompany?.website ??
			clearbitPrimary?.website ??
			`https://${q.toLowerCase().replace(/\s+/g, "")}.com`;
		...
		console.log(
			`[People] Hunter: ${hunterData.people.length}, GitHub: ${githubPeople.length}, LinkedIn: ${linkedInPeople.length}, merged: ${mergedPeople.length}`,
		);
		...
		if (firecrawlResult?.bestDescription) {
			console.log(
				`[Firecrawl] "${firecrawlResult.bestDescription.slice(0, 80)}…"`,
			);
		}
		...
		if (ycCompany) {
			...
			const firecrawlDesc =
				firecrawlResult?.ogDescription || firecrawlResult?.metaDescription;
			const ycDesc =
				ycCompany.long_description?.trim().slice(0, 1000) ||
				ycCompany.one_liner ||
				"";
			const description = firecrawlDesc || ycDesc;

			const logo_url =
				ycCompany.small_logo_thumb_url ??
			...
			let company: Record<string, unknown> & { id: string };

			if (existingYC) {
				// Already in DB — update description + logo in case they improved
				console.log(
					`[Search] YC company already cached, updating: ${ycCompany.name}`,
				);
				...
				if (insertErr || !inserted) {
					...
					return NextResponse.json(
						{ error: "db_error", message: "Failed to save company." },
						{ status: 500 },
					);
				}
				company = inserted;
			...
			if (realPeople.length > 0) {
				...
				if (pe) console.error("[Search] People insert error:", pe.message);
					...
					console.log(
						`[Search] Saved ${people.length} real people (Hunter+GitHub+LinkedIn)`,
					);
			} else {
				console.log(
					"[Search] No people found from any source — leaving team empty",
				);
			}
		...
		if (webResult) {
			console.log(
				`[Search] Web: ${clearbitAll.length} Clearbit result(s) — primary: ${webResult.name} | ${webResult.domain}`,
			);
			...
			return NextResponse.json(
				{
					...
					message: `Couldn't find "${q}". Try the exact company name or check your spelling.`,
				},
				{ status: 404 },
			);
		}
		...
		if (webResult?.domain && domain !== webResult.domain) {
			...
			if (retry?.bestDescription) {
				console.log(
					`[Firecrawl] Retry with correct domain: "${retry.bestDescription.slice(0, 80)}…"`,
				);
				bestFirecrawl = retry;
			}
		}
		...
		try {
			...
			const msg =
				aiErr instanceof Error
					? aiErr.message
					: typeof aiErr === "object" && aiErr !== null && "message" in aiErr
						? String((aiErr as { message: unknown }).message)
						: String(aiErr);
			const status =
				typeof aiErr === "object" && aiErr !== null && "status" in aiErr
					? (aiErr as { status: unknown }).status
					: undefined;
			if (
				status === 429 ||
				msg.includes("429") ||
					...
					return NextResponse.json(
						{
							error: "rate_limited",
							message:
								"AI rate limit hit. Please wait ~60 seconds and try again.",
						},
						{ status: 429 },
					);
				}
		...
		const finalMission =
			(bestFirecrawl?.ogDescription || rawDescription)?.split(".")[0]?.trim() +
				"." || "";
		const finalIndustry = aiMeta?.industry || linkedInCompany?.industry || "";
		const finalLogo =
			webResult?.logo_url ||
			(webResult?.domain
				? `https://favicon.im/${webResult.domain}?larger=true`
				: null) ||
			(domain ? `https://favicon.im/${domain}?larger=true` : null);

		console.log(
			`[Search] Storing non-YC company: name="${finalName}" desc="${finalDesc.slice(0, 60)}…"`,
		);
		...
		if (existingCompany) {
			...
			const { data: updated, error: updateErr } = await supabaseAdmin
				...
				.update({
					...
					technologies: aiMeta?.technologies?.length
						? aiMeta.technologies
						: existingCompany.technologies,
				})
		...
		console.log(
			`[Search] Company ready: id=${company.id} name="${company.name}"`,
		);
		...
		if (realPeople.length > 0) {
			...
			if (pe) console.error("[Search] People insert error:", pe.message);
				...
				console.log(
					`[Search] Saved ${realPeople.length} real people (Hunter+GitHub+LinkedIn)`,
				);
		} else {
			console.log(
				"[Search] No people found from any source — leaving team empty",
