...
export async function POST(req: NextRequest) {
	try {
		...
		console.log(
			`[Generate] Context (${context.fromCache ? "cached" : "fresh"}):\n${context.compact}`,
		);
		...
		return NextResponse.json({
			...
			contextSignals: {
				...
				phProduct: context.phProducts?.[0]
					? `${context.phProducts[0].name} (${context.phProducts[0].votesCount} votes)`
					: null,
		...
		if (status === 429 || msg.includes("429") || msg.includes("quota")) {
			return NextResponse.json(
				{
					error: "rate_limited",
					message: "AI rate limit. Wait ~60s and retry.",
				},
				{ status: 429 },
			);
		}
