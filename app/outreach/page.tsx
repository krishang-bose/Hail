...
function OutreachContent() {
	...
	const [errorType, setErrorType] = useState<"generic" | "daily_limit">(
		"generic",
	);
	...
	const handleGenerate = async () => {
		...
		try {
			...
			setResult({
				...
				usage: data.usage,
				contextSignals: data.contextSignals,
			});
		...
		>
			...
			<main className="flex-1 max-w-3xl mx-auto w-full px-6 py-10">
				...
				<div className="mb-8 fade-up">
					<h1 className="text-2xl font-semibold text-[var(--brown-3)] mb-1">
						Generate your message
					</h1>
					<p className="text-sm text-[var(--muted)]">
						For{" "}
						<span className="text-[var(--brown-3)] font-medium">
							{personName}
						</span>{" "}
						at{" "}
						<span className="text-[var(--brown-3)] font-medium">
				...
				<div className="surface p-5 mb-5 flex flex-col sm:flex-row gap-4 fade-up delay-1">
					<div className="flex items-center gap-3 flex-1">
						...
						<div>
							...
							<p className="font-medium text-sm text-[var(--brown-3)]">
								{personName}
							</p>
				...
				{AUTH_ENABLED && status !== "loading" && !session ? (
					<div className="surface p-8 mb-5 text-center fade-up delay-2">
						...
						<h2 className="text-base font-semibold text-[var(--brown-3)] mb-2">
							Sign in to generate
						</h2>
						<p className="text-sm text-[var(--muted)] mb-5 leading-relaxed max-w-xs mx-auto">
							Free account required. You get{" "}
							<strong className="text-[var(--brown-2)]">
								5 personalized messages per day
							</strong>{" "}
							— free forever.
						</p>
						<button
					...
					<div className="surface p-6 mb-5 fade-up delay-2">
						<div className="flex items-center justify-between mb-4">
							<p className="text-sm font-medium text-[var(--brown-3)]">
								What&apos;s your goal?
							</p>
						...
						>
							{loading ? (
								<>
									<Loader2 className="w-4 h-4 animate-spin" /> Gathering context
									&amp; generating…
								</>
							) : isExhausted ? (
				...
				{error && (
					<div
						className="mb-5 px-4 py-3 rounded-xl border text-sm flex items-start gap-3"
						style={{
					...
					>
						...
						<div>
							<p className="font-medium">
								{errorType === "daily_limit" ? "Daily limit reached" : "Error"}
							</p>
							...
							<div className="flex flex-wrap gap-2 items-center">
								...
								{result.contextSignals.jobs.length > 0 && (
									...
									>
										...
										{result.contextSignals.jobs.length} open roles
									</span>
								)}
							...
							<div className="surface overflow-hidden">
								...
								>
									<div className="flex items-center gap-2">
										...
										<span className="text-xs font-medium text-[var(--brown-3)]">
											LinkedIn DM
										</span>
									</div>
									<button
										onClick={() => copy(result.linkedin, "li")}
								...
								<div className="p-5">
									<p className="text-sm text-[var(--brown-3)] leading-relaxed whitespace-pre-wrap">
										{result.linkedin}
									</p>
								</div>
							</div>
