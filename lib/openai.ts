...
/** Safely check if an unknown error looks like a quota/rate-limit error. */
...
async function callAI(prompt: string): Promise<string> {
	...
	try {
		...
		errors.push(`Gemini: ${isQuotaError(e) ? "quota exceeded" : msg}`);
	...
	try {
				...
				messages: [
					{
						role: "system",
						content:
							"You are a JSON-only assistant. Always respond with valid JSON only, no markdown, no explanation.",
					},
		...
		errors.push(`Groq: ${toErrorMessage(e)}`);
	...
	try {
					...
					messages: [
						{
							role: "system",
							content:
								"You are a JSON-only assistant. Always respond with valid JSON only.",
						},
						{ role: "user", content: prompt },
					],
					max_tokens: 2000,
		...
		errors.push(`Together AI: ${toErrorMessage(e)}`);
	...
	try {
		...
		errors.push(`Cerebras: ${toErrorMessage(e)}`);
	...
	try {
					...
					messages: [
						{
							role: "system",
							content:
								"You are a JSON-only assistant. Always respond with valid JSON only, no markdown.",
						},
						{ role: "user", content: prompt },
					],
					max_tokens: 2000,
		...
		errors.push(`Cohere: ${toErrorMessage(e)}`);
...
async function generatePeople(params: {
	companyName: string;
	industry: string;
	batch?: string;
