...
export async function buildPersonContext(params: {
	...
	const [github, jobs, hnMentions, phProducts] = await Promise.all([
		...
		domain
			? fetchJobPostings(domain).catch(() => [])
			: Promise.resolve<JobPosting[]>([]),
	...
	const personHN = hnMentions
		.filter((h) =>
			h.toLowerCase().includes(personName.split(" ")[0].toLowerCase()),
		)
		.slice(0, 3);
...
function buildCompact(p: {
	...
	const prev = p.personExperience
		? ` | prev: ${p.personExperience.slice(0, 100)}`
		: "";
	...
	if (p.github) {
		...
		lines.push(
			`GITHUB: @${p.github.username}${ghBio} | langs: ${langs} | repos: ${repos}`,
		);
	}
