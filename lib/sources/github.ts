...
export async function findGitHubProfile(
	...
	for (const r of repos ?? []) {
		...
		if (r.language)
			langScore[r.language] =
				(langScore[r.language] ?? 0) + r.stargazers_count + 1;
		recentRepos.push({
			name: r.name,
...
function deriveRole(bio: string | null, company: string | null): string {
	...
	if (text.includes("vp eng") || text.includes("vp of eng"))
		return "VP Engineering";
	if (text.includes("design") || text.includes("ux ") || text.includes("ui "))
		return "Designer";
	if (text.includes("product manager") || text.includes("product lead"))
		return "Product Manager";
	if (text.includes("devrel") || text.includes("developer advocate"))
...
export async function searchGitHubOrg(
	...
	if (!orgLogin) {
		console.log(
			`[GitHub] No org found for "${companyName}" (tried: ${slugs.join(", ")} + search)`,
		);
		return [];
	}

	type GithubMember = { login: string };
	...
	const profiles = await Promise.all(
		members.slice(0, limit * 2).map(async (m): Promise<GitHubPerson | null> => {
			const user = (await ghFetch(
				`https://api.github.com/users/${m.login}`,
			)) as GithubUserProfile | null;
