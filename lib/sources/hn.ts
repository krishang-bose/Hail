...
export async function searchHN(query: string): Promise<string[]> {
	try {
		...
		const relevant = hits.filter(
			(h) => h.title?.toLowerCase().includes(q) && h.points > 10,
		);
...
async function searchHNHiring(companyName: string): Promise<string[]> {
	try {
		...
		return (hits as HNHiringHit[])
			.filter((h) => h.comment_text?.toLowerCase().includes(q))
			.slice(0, 2)
			.map((h) => {
